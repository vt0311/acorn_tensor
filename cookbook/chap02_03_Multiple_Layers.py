...
@author: acorn
'''
# Working with Multiple Layers (쿡북69)
# CNN 코드
...
x_shape = [1, 4, 4, 1]  # 이미지 갯수, 높이, 너비, 채널수
...
my_filter = tf.constant(0.25, shape=[2, 2, 1, 1]) # 2*2 의 필터 
my_strides = [1, 2, 2, 1]
mov_avg_layer= tf.nn.conv2d(x_data, my_filter, my_strides,
                            padding='SAME', name='Moving_Avg_Window')

# stride = 2 이므로 4, 4 가 2, 2 가 된다.

# x_data         my_filter         결과
# (1, 4, 4, 1)   (2, 2, 1, 1)      (1, 2, 2, 1)


# squeeze : shape에서 크기가 1인 항목들을 모두 제거한다.
# t의 shape가 shape(1, 2, 2, 1)이라고 할때, tf.squeeze(t)라고 하면
# shape(2, 2)가 된다.
